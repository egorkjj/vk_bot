        ...
        try:
            ...
            def sent_for_th(user_id, user_name):
                ...
                if user_step_check(user_name) != 7:
                    ...
                    links = links_text()
                    write_msg(user_id, f"Добрый день, меня зовут Сергей, я Ваш менеджер компании WorkPoint, ранее Вы интересовались онлайн заработком, поэтому высылаю вам актуальные задания, все что нужно сделать это оформить карты по ссылкам ниже и сообщить об оформлении:\n{links}", None)
                else:
                    time.sleep(86400)
                    if user_step_check(user_name) != 10:
                        disable_messages(user_name)
                        write_msg(user_id, f"Добрый день, меня зовут Сергей, я менеджер компании WorkPoint, ранее Вы получили список заданий, подскажите, удалось ли оформить карты?", None)
